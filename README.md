...
This plugin ships 26 skills that phase composition selects automatically. All but `unified-context-stack` (which registers as a plugin) are listed in `config/default-triggers.json` and discoverable at session start. Invoke one explicitly when the routing hasn't picked it up yet.
...
| PLAN | [second-opinion](skills/second-opinion/SKILL.md) | ONE other model's view — independent opinion (your prior answer withheld) or critique (deliberately included) |
| PLAN | [panel](skills/panel/SKILL.md) | Independent cross-family perspectives from SEVERAL models — single round, verbatim attributed delivery |
