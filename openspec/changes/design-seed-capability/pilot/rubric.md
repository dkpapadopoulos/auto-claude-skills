...
The rubric; both artifacts (HTML source plus matched light and dark
screenshots); and the frozen source envelope the artifacts were built from.
Both artifacts were built from the identical envelope, so having it tells you
nothing about which artifact is which — it is there so R3, R7 and R8 can be
checked against the actual supplied data rather than guessed from the
screens.
...
| R3 | Nothing is fabricated, interpolated, or inferred beyond the supplied data. Checked against the envelope, not inferred from the screens alone. | Host repo principle: "never invent prices, returns, or holdings" |
...
| R7 | Honest handling of an empty collection or section: when the envelope supplies one as explicitly empty rather than populated (e.g. `drift_analysis.snapshot_drift` is `{}` beside a `proposal_drift` with five populated entries; `optimizer_summary.policy_violations` is `[]`), that empty state is communicated to the reader as a deliberate zero, not silently dropped from the screen. Checked against the envelope, not inferred from the screens alone. Excellent (4) handling also states plainly that the report omits some section entirely (e.g. `comparators`, `integrity_evidence` — fields on the underlying dataclass but absent from every key in this envelope); that is one mark of the top score, not a separately required or separately weighted check. | Independent reviewer's stated criterion; empty-collection and absent-section instances verified directly against the frozen fixture |
| R8 | Fidelity: every displayed value traces to the supplied data. Checked against the envelope, not inferred from the screens alone. | This pilot |
