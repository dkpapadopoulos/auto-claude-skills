...
1. **CI Status** (fail-closed: absent ≠ green)
   ...
   **Local verification of record:** when hosted CI is absent, you MAY accept a fresh `~/.claude/.skill-project-verified-<token>` evidence file as verification performed on substrate `local` ONLY when its `failed` list is empty, its `could_not_verify` list is empty, `gate_gaming_status` is **exactly `clean`**, AND `coverage_adequacy_status` is **not `suspect`**. Any other value for `gate_gaming_status` — `suspect` (a possibly-gamed gate), `unverified` (the gate-gaming check could not run), or the field **absent** — MUST NOT be accepted: require the positive `clean` assertion, because absence is not proof (just as a non-empty `could_not_verify`, a gate that could not run, is not).

   `coverage_adequacy_status` has a deliberately different acceptance rule than `gate_gaming_status`, in parity with `project-verification`'s own tri-state contract for the field. A `suspect` value (new code shipping below the changed-line coverage floor) BLOCKS acceptance the same way `gate_gaming_status: suspect` does — both are positive evidence of a problem, not mere absence of proof. But `unverified` or an absent `coverage_adequacy_status` does NOT block acceptance, unlike an absent `gate_gaming_status`. This asymmetry is intentional: `project-verification`'s adequacy check is itself fail-open by contract — `unverified` (no coverage tooling present, or an unparseable artifact) MUST NOT block there — so treating an absent or `unverified` field as a rejection here would silently re-impose a block the upstream check explicitly disclaims, penalizing repos that simply lack coverage tooling rather than repos with a demonstrated coverage gap.

   Since absence-of-failure is not proof-of-pass, when rejecting evidence surface the specific reason (which gates could not be verified, that gate-gaming was detected/unverified in the diff, or that `coverage_adequacy_status` was `suspect`) rather than only reporting that hosted CI was absent. Still surface that hosted CI was absent rather than claiming hosted-CI green. This evidence is advisory provenance, not a non-bypassable gate.
