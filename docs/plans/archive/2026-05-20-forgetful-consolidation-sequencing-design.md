...
**Acceptance Scenarios:**
- [x] Phase doc states consolidation precedes `git push` with rationale — implemented as designed (`skills/unified-context-stack/phases/ship-and-learn.md:34`).
- [x] Forgetful query returns recovered learnings with provenance — implemented as designed (Forgetful ids 6 and 7, project Dion id 2, `source_repo=damianpapadopoulos/Dion`, confidence 0.85).
- [x] Kill criterion findable in design doc + per-project memory + Forgetful — implemented as designed (Forgetful id 8, `project_forgetful_consol_kill_criterion.md`, `MEMORY.md` index line 57).
- [ ] No hard guard added on the 0–1-miss path — not yet evaluable; review at 2026-06-17.

**Scope changes:**
- Added: created a new `Dion` Forgetful project (id 2) to scope the two recovered memories. Not called out in the original design's Capabilities Affected because the design treated the recovery as a data action outside the repo. Captured here for traceability.
- Removed: none.
- Modified: none.

**Design decision changes:**
- The shipped phase-doc wording is slightly stronger than the design's: design said "re-entering the gate path tends to drop the consolidation step"; shipped doc says "the operator path back to the consolidation step is fragile — recovery typically drops it." Meaning equivalent; shipped wording is more actionable. No design revisit needed.
- Auto-linking quirk in Forgetful: ids 6 and 7 were auto-linked to several unrelated auto-claude-skills memories (ids 1-3). Left in place — noise but harmless. Not a design concern.
