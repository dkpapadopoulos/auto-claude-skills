...
find "${HOME}/.claude" -maxdepth 1 \
    \( -name '.skill-composition-state-*' -o -name '.skill-openspec-state-*' \
       ...
       -o -name '.skill-reviewer-saturated-*' -o -name '.skill-egress-*' \) \
    ...
    ! -name ".skill-egress-*-${_SESSION_TOKEN}.*" \
    ...
    DEFAULT_JSON="$(printf '%s' "${DEFAULT_JSON}" | jq '
      (.phase_compositions.DESIGN.hints // []) |= map(
        if (.text // "" | test("PERSIST DESIGN")) then
          .text = "PERSIST DESIGN (spec-driven): Create `openspec/changes/<feature-slug>/proposal.md`, `openspec/changes/<feature-slug>/design.md`, and `openspec/changes/<feature-slug>/specs/<capability-slug>/spec.md` upfront after brainstorming approval. Sections in proposal: Why, What Changes, Capabilities (Added/Modified), Impact. Sections in design: Architecture, Trade-offs, Dissenting views, Decisions. Spec file uses RFC 2119 UPPERCASE keywords and 2-4 GIVEN/WHEN/THEN acceptance scenarios. Then run, in ONE Bash call: bash \"${CLAUDE_PLUGIN_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null)}/scripts/persist-state.sh\" upsert-change \"$SLUG\" \"\" \"\" \"$CAPABILITY_SLUG\" \"\" to set change_slug + capability_slug in session state — the script resolves the session token internally (issue #157), so you author only the slug and capability. If introducing a new capability, emit a visible NEW CAPABILITY warning for user review."
