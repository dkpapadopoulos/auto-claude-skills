    ...
    DEFAULT_JSON="$(printf '%s' "${DEFAULT_JSON}" | jq '
      (.phase_compositions.DESIGN.hints // []) |= map(
        if (.text // "" | test("PERSIST DESIGN")) then
          .text = "PERSIST DESIGN (spec-driven): Create `openspec/changes/<feature-slug>/proposal.md`, `openspec/changes/<feature-slug>/design.md`, and `openspec/changes/<feature-slug>/specs/<capability-slug>/spec.md` upfront after brainstorming approval. Sections in proposal: Why, What Changes, Capabilities (Added/Modified), Impact. Sections in design: Architecture, Trade-offs, Dissenting views, Decisions. Spec file uses RFC 2119 UPPERCASE keywords and 2-4 GIVEN/WHEN/THEN acceptance scenarios. Then run, in ONE Bash call (shell state does not persist between tool calls): PR=\"${CLAUDE_PLUGIN_ROOT:-$(git rev-parse --show-toplevel)}\"; TOKEN=\"$(. \"$PR/hooks/lib/session-token.sh\" 2>/dev/null && resolve_own_session_token || cat ~/.claude/.skill-session-token 2>/dev/null)\"; . \"$PR/hooks/lib/openspec-state.sh\"; openspec_state_upsert_change \"$TOKEN\" \"$SLUG\" \"\" \"\" \"$CAPABILITY_SLUG\" \"\" to set change_slug + capability_slug in session state. Resolve $TOKEN exactly that way — never by reading ~/.claude/.skill-session-token directly: it is a shared last-writer-wins singleton that under concurrent sessions names a DIFFERENT conversation, so the state lands where the payload-first readers never look (issue #157). If introducing a new capability, emit a visible NEW CAPABILITY warning for user review."
